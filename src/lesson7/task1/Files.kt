...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    for (name in substrings) {
        ...
        var member = Regex(name.toLowerCase()).find(read)
        while (member != null) {
            member = Regex(name.toLowerCase()).find(read, member.range.first + 1)
...
fun sibilants(inputName: String, outputName: String) {
    ...
    var previousElement = ' '
    var everyElement = input.read()
    val consonants = listOf('Ж', 'ж', 'Ч', 'ч', 'Ш', 'ш', 'Щ', 'щ')
    val vowels = mapOf('Ы' to 'И', 'ы' to 'и', 'Я' to 'А', 'я' to 'а', 'Ю' to 'У', 'ю' to 'у')
    while (everyElement != -1) {
        var currentElement = everyElement.toChar()
        if (previousElement in consonants && currentElement in vowels) currentElement = vowels[currentElement]!!.toChar()
        previousElement = currentElement
        everyElement = input.read()
        output.write(currentElement.toInt())
    }
    input.close()
    output.close()
}
    ...
    for (line in File(inputName).readLines().map { it.trim() }) {
        ...
        if (line.replace(Regex("""\s+"""), " ").length == maxLineLength || words <= 1)
            ...
            for (i in 0 until list.size) {
                ...
                if (residualSpaces > i) output.write(" ")
...
fun top20Words(inputName: String): Map<String, Int> {
    ...
    for (text in File(inputName).readLines()) {
        for (word in Regex("""[a-zа-яё]+""").findAll(text.toLowerCase())) list.add(word.value)
    }
    for (word in list) {
        if (map[word] == null) map[word] = 1 else map[word] = map[word]!! + 1
    }
    return if (map.toList().size <= 20) map.toList().sortedByDescending { it.second }.toMap()
    else map.toList().sortedByDescending { it.second }.subList(0, 20).toMap()
}
